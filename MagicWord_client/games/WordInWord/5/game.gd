...
#——————————————————————————————以下是要手动设置的参数————————————————————————————
#表示这个游戏在这类游戏中是第几个
var gameOfPosition = 5
var path = "res://asset/images/wordInword/g5/b/"
var path_pressed = "res://asset/images/wordInword/g5/w/"
#表示这个字的可以被点击的笔画数
var brush_count = 10
#代表一共可以组成多少个字
var max_count = 10
...
func check(isPressed):
	...
	match(downCount):
		1:
			if(cd[0] in [1,3,10]):
				return "丨"
			if(cd[0] in [2,4,5,7,8,9]):
				return "一"
		2:
			if(cd[0] in [2,4,5,7,8,9] && cd[1] in [2,4,5,7,8,9]):
				return "二"
			if(cd[0] in [7,8,9] && cd[1] == 10):
				return "十"
		3:
			if(cd[0] in [2,4,5,7,8,9] && cd[1] in [2,4,5,7,8,9]):
				if(cd[2] in [2,4,5,7,8,9]):
					return "三"
				elif(cd[0] == 8 && cd[1] == 9 && cd[2] == 10):
					return "土"
				elif(cd[0] == 8 && cd[1] == 9 && cd[2] == 10):
					return "土"
		4:
			if(cd[0] in [2,4,5,7,8,9] && cd[1] in [2,4,5,7,8,9] && cd[2] in [2,4,5,7,8,9] && cd[3] in [2,4,5,7,8,9]):
				print(1)
				return "亖"
			if(cd == [1,2,3,5]):
				return "口"
		...
		5:
			...
			if(cd == [6,7,8,9,10]):
				return "生"
		6:
			if(cd == [1,2,3,4,5,7] || cd == [1,2,3,4,5,8]):
				return "旦"
		7:
			if(cd == [1,2,3,4,5,7,10] || cd == [1,2,3,4,5,8,10]):
				return "早"
		8:
			if(cd == [1,2,3,4,5,7,8,10]):
				return "旱"
		10:
			return "星"
	return null

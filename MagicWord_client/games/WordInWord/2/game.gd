...
#——————————————————————————————以下是要手动设置的参数————————————————————————————
#表示这个游戏在这类游戏中是第几个
var gameOfPosition = 2
var path = "res://asset/images/wordInword/g2/b/"
var path_pressed = "res://asset/images/wordInword/g2/w/"
#表示这个字的可以被点击的笔画数
var brush_count = 5
#代表一共可以组成多少个字
var max_count = 1
	...
	match(downCount):
		1:
			...
			if(cd[0] == 5):
				return "丶"
		2:
			if(cd[0] in [1,2,3] && cd[1] in [1,2,3]):
				return "二"
			if(cd == [4,5]):
				return "卜"
			if(cd == [2,4]):

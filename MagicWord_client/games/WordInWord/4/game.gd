...
#——————————————————————————————以下是要手动设置的参数————————————————————————————
#表示这个游戏在这类游戏中是第几个
var gameOfPosition = 4
var path = "res://asset/images/wordInword/g4/b/"
var path_pressed = "res://asset/images/wordInword/g4/w/"
#表示这个字的可以被点击的笔画数
var brush_count = 9
#代表一共可以组成多少个字
var max_count = 10
	...
	match(downCount):
		1:
			...
			if(cd[0] in [4,6,7,8]):
				return "一"
		2:
			if(cd[0] in [4,6,7,8] && cd[1] in [4,6,7,8]):
				return "二"
			if(cd[0] in [6,7,8] && cd[1] == 9):
				return "十"
		3:
			if(cd[0] in [4,6,7,8] && cd[1] in [4,6,7,8]):
				if(cd[2] in [4,6,7,8]):
					return "三"
			if(cd[0] == 4 && cd[1] in [6,7] && cd[2] == 9):
					return "干"
		4:
			if(cd == [4,6,7,8]):
				return "亖"
